  ...
  try {
    ...
    if (currentUser) {
      await createAuditLog({
        ...
        entityType: 'SERVICE',
        entityId: service.id,
        entityName: `${serviceType}${description ? ` - ${description}` : ''}`,
        newValue: JSON.stringify({ serviceType, description, monthlyPrice, status }),
        description: `Servicio "${serviceType}" añadido a ${client.company}`,
      });
    }
